...
static const CGFloat ZSYCustomButtonHeight = 44;

static const char * const kPopupListButtonClickForCancel = "kPopupListButtonClickForCancel";

static const char * const kPopupListButtonClickForDone = "kPopupListButtonClickForDone";

@interface PopupListView ()
@property (nonatomic, retain) UITableView *mainPopupListView;                 //主的选择列表视图
...
@synthesize mainPopupListView = _mainPopupListView;
...
//    [self addSubview:self.titleName];
    ...
    _mainPopupListView = [[UITableView alloc] initWithFrame:tableFrame style:UITableViewStylePlain];
    self.mainPopupListView.dataSource = self;
    self.mainPopupListView.delegate = self;
    [self addSubview:self.mainPopupListView];
}

- (void)refreshTheUserInterface
{
    self.mainPopupListView.rowHeight = 32.0f;
    
    if (self.cancelButton || self.doneButton)
    {
        self.mainPopupListView.frame = CGRectMake(0, 32.0f, self.mainPopupListView.frame.size.width, self.mainPopupListView.frame.size.height - ZSYCustomButtonHeight);
    }
...
{
    return [self.mainPopupListView indexPathForSelectedRow];
}
#pragma mark - UITableViewDatasource

- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    if (self.datasource && [self.datasource respondsToSelector:@selector(popupListView:numberOfRowsInSection:)])
    {
        return [self.datasource popupListView:self numberOfRowsInSection:section];
    }
    return 0;
}
...
{
    if (self.datasource && [self.datasource respondsToSelector:@selector(popupListView:cellForRowAtIndexPath:)])
    {
        return [self.datasource popupListView:self cellForRowAtIndexPath:indexPath];
    }
    return nil;
}
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(popupListView:didDeselectRowAtIndexPath:)])
    {
        [self.delegate popupListView:self didDeselectRowAtIndexPath:indexPath];
    }
}

- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(popupListView:didSelectRowAtIndexPath:)])
    {
        [self.delegate popupListView:self didSelectRowAtIndexPath:indexPath];
    }
}
...
{
    return [self.mainPopupListView dequeueReusableCellWithIdentifier:identifier];
}

- (UITableViewCell *)popupCellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    return [self.mainPopupListView cellForRowAtIndexPath:indexPath];
}
...
- (void)setCancelButtonTitle:(NSString *)aTitle block:(PopupListViewButtonBlock)block
{
    if (nil == _cancelButton)
    {
        ...
        [self.cancelButton setBackgroundImage:[PopupListView cancelButtonBackgroundImage] forState:UIControlStateNormal];
    ...
    objc_setAssociatedObject(self.cancelButton, kPopupListButtonClickForCancel, [block copy], OBJC_ASSOCIATION_RETAIN);
}
...
- (void)setDoneButtonWithTitle:(NSString *)aTitle block:(PopupListViewButtonBlock)block
{
    if (nil == _doneButton)
    {
        ...
        [self.doneButton setBackgroundImage:[PopupListView normalButtonBackgroundImage] forState:UIControlStateNormal];
    ...
    objc_setAssociatedObject(self.doneButton, kPopupListButtonClickForDone, [block copy], OBJC_ASSOCIATION_RETAIN);
}
#pragma mark - UIButton Clicke Method
- (void)buttonWasPressed:(id)sender
{
    UIButton *button = (UIButton *)sender;
    PopupListViewButtonBlock __block block;
    
    if (button == self.cancelButton)
    {
        block = objc_getAssociatedObject(sender, kPopupListButtonClickForCancel);
    }
    else
    {
        block = objc_getAssociatedObject(sender, kPopupListButtonClickForDone);
    }
    if (block)
    {
        block();

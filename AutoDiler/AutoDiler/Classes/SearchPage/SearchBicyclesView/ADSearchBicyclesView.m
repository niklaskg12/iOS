...
@implementation ADSearchBicyclesView
...
- (void)initView
{
    ...
    self.tvModel.text = @"";
    [self.typeBicycleBtn setTitle:[NSString stringWithFormat:@"   %@", self.model.bicTypes[0]] forState:UIControlStateNormal];
...
- (IBAction)onSearchItemBtnClick:(id)sender {
    ...
    } else if (tag == 92  ||  tag == 93) {
        self.model.curSearchItem = 100;
        self.model.searchItems = self.model.prices;
    } else if (tag == 94  ||  tag == 95) {
        self.model.curSearchItem = 100;
